...
int LabelTable::labelIsDefined(const string& token, int value)
{
	...
	if (index == -1) {
		...
		if (label_table[index].value == -1) {
			...
			std::cout << "Label " << addQuotes(token) << " is defined more than once - first occurrence used\n";
		}
	}
	...
	if (num_labels == 0) {
		...
		for (int i{ 0 }; i < num_labels; i++) {
			std::cout << std::right << std::setw(8)  << i << "    "
				 << std::left  << std::setw(24) << addQuotes(label_table[i].name)
				 << std::right << std::setw(8)  << label_table[i].value << '\n';
		}
	}
}
...
int LabelTable::addLabel(const string& token, int value)
{
	...
	if (num_labels < MAX_LABELS) {
		...
		std::cout << "MAX_LABELS (" << MAX_LABELS << ") exceeded\n";
	}
