	...
	for (int i{ 0 }; i < n; i++) {
		...
		if (complete_token) {
			if (num_tokens < MAX_TOKENS) {
				...
				std::cout << "Too many operand(s) in line " << addQuotes(line) << '\n';
				parse_success = false;
			}
			num_tokens++;
			token.clear();
	...
	if (num_tokens > 0) {
		if (!addLabelOrInstruction(tokens)) {
			if (isOpcode(tokens[0])) {
				std::cout << "Invalid or missing operand(s) ";
			}
			else if (isLabelDefinition(tokens[0])) {
				std::cout << "Invalid label definition ";
			}
			else {
				std::cout << "Unrecognized opcode ";
			}
			std::cout << "in line " << addQuotes(line) << '\n';
			parse_success = false;
		}
	}
}
	...
	if (num_instructions == 0) {
		...
		for (int i{ 0 }; i < num_instructions; i++) {
			...
			if (validInstructionOperands(i)) {
				std::cout << "            " << indexToInstructionLine(i) << '\n';
			}
		}
	}
}
...
string FlightPlanParse::indexToInstructionLine(int index) const
{
	...
	case Opcodes::CMP:
		...
		if (instruction.constant_operand2) {
			result += std::to_string(instruction.operand2);
		}
		else {
...
bool FlightPlanParse::validInstructionOperands(int index) const
{
	...
	case Opcodes::CMP:
		if ((!int_variable_table.validIndex(instruction.operand1)) ||
			...
			std::cout << "Undeclared integer variable(s) used in ";
	...
	if (!valid_operands) {
		std::cout << opcodeToString(instruction.opcode) << " instruction at location " << index
